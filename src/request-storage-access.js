...
(function () {
  ...
  const initStorageParams = (params = {}) => {
    ...
    storageParams.canRequest = !(
      storageParams.canRequest === "false" || storageParams.canRequest === false
    );
  };
  ...
  const getlandAtPath = () => {
    if (!originalCCPUrl) {
      throw new Error(
        "[StorageAccess] [getlandAtPath] Invoke connect.storageAccess.init first"
      );
    }
  ...
  const getRequestStorageAccessUrl = () => {
    ...
    if (isConnectDomain(originalCCPUrl)) {
      ...
      storageParams.instanceUrl &&
      isConnectDomain(storageParams.instanceUrl)
    ) {
      connect
        .getLog()
        .info(
      ...
      storageParams.instanceUrl &&
      isLocalhost(storageParams.instanceUrl)
    ) {
      connect
        .getLog()
        .info(`[StorageAccess] [getRequestStorageAccessUrl] Local testing`);
    ...
    function handleUpstreamMessages({ data, source }) {
      ...
      switch (data.event) {
        case storageAccessEvents.INIT: {
          connect
            .getLog()
            .info(`[StorageAccess][INIT] message recieved`)
            .withObject(data);
          if (onInit) {
            onInit(data);
          }
        ...
        case storageAccessEvents.GRANTED: {
          connect
            .getLog()
            .info(`[StorageAccess][GRANTED] message recieved`)
            .withObject(data);
          if (onGrant) {
            onGrant(data);
          }
        ...
        case storageAccessEvents.DENIED: {
          connect
            .getLog()
            .info(`[StorageAccess][DENIED] message recieved`)
            .withObject(data);
          if (onDeny) {
            onDeny(data);
          }
